...
# fuse the two vectors (support features and query features)
def catEmbeding(a, b):
    n = a.shape[0]
    m = b.shape[0]
...
# Relation function to estimate the similarity for fused vectors, output the probability of the sample belonged to each category
class Gx(nn.Module):
...
def fast_adapt(model, batch, ways, shot, query_num, metric=None, device=None):
    if metric is None:
        metric=catEmbeding # choose the similarity metric
    if device is None:
        device = model.device()
